...
\midrule
House Price Index & 7.7 & 53.5 & 15.0 & 215.5 & 63.5\\
Monthly Sales (units) & 45,218.0 & 126,019.4 & 120,437.5 & 265,223.0 & 35,471.8\\
USD/TRY Rate & 2.1 & 13.4 & 6.4 & 44.5 & 13.3\\
EUR/TRY Rate & 2.7 & 14.9 & 7.2 & 51.7 & 14.8\\
\bottomrule
\end{longtable}

The HPI has grown from around 100 to several thousand, reflecting the
steep price escalation over the decade. The USD/TRY rate rose from
roughly 2 to over 30, and monthly sales volumes are in the tens of
thousands but show high variability.
...
Retail trade, food, and e-commerce dominate credit card spending.
Construction and furniture categories, while smaller, are directly
...
The HPI grew modestly through 2019, then accelerated sharply from 2020
onward, with the steepest rise concentrated between 2021 and 2023 ---
precisely mirroring the most severe TRY depreciation episodes.
...
The YoY chart is more telling than raw levels: the two series spike
together, particularly in 2018, 2021, and 2022. When the exchange rate
surged by over 80\% in 2021--2022, house price growth followed almost
immediately --- consistent with a direct cost pass-through mechanism.
...
\FunctionTok{ggplot}\NormalTok{(ks\_turkey, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Date, }\AttributeTok{y =}\NormalTok{ Total\_Sales)) }\SpecialCharTok{+}
  \FunctionTok{geom\_col}\NormalTok{(}\AttributeTok{fill =} \StringTok{"\#74add1"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.85}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_x\_date}\NormalTok{(}\AttributeTok{date\_breaks =} \StringTok{"2 years"}\NormalTok{, }\AttributeTok{date\_labels =} \StringTok{"\%Y"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_y\_continuous}\NormalTok{(}\AttributeTok{labels =} \FunctionTok{label\_comma}\NormalTok{(}\AttributeTok{accuracy =} \DecValTok{1}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Figure 4. Monthly Housing Sales – Türkiye"}\NormalTok{,}
...
Sales volumes show strong seasonality but have not risen with prices.
Post-2022 softening is consistent with buyers being priced out of the
market as affordability deteriorated.
...
All categories grew sharply in nominal terms from 2020 onward,
reflecting the broad inflationary environment which also raises
construction costs, labour, and land values.
...
\NormalTok{corr\_df }\OtherTok{\textless{}{-}}\NormalTok{ corr\_df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{Var1 =} \FunctionTok{factor}\NormalTok{(Var1, }\AttributeTok{levels =}\NormalTok{ var\_order),}
...
The heatmap confirms the narrative: HPI correlates near-perfectly with
both exchange rates (r \textgreater{} 0.97) and credit card expenditure
(r \textgreater{} 0.95), all driven by the same underlying monetary
inflation. The HPI--Sales correlation is weakly negative, reflecting
demand compression at high price levels.
...
\pandocbounded{\includegraphics[keepaspectratio]{project_files/figure-pdf/scatter-hpi-usd-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{project_files/figure-pdf/regional-bar-1.pdf}}
...
To formally quantify the drivers of housing price growth, we estimate a
multiple linear regression model. Log-transformations are applied to all
variables to handle exponential growth and to interpret coefficients as
elasticities.
...
\NormalTok{  ) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{caption =} \FunctionTok{sprintf}\NormalTok{(}\StringTok{"Table 5. Baseline Model – R² = \%.3f"}\NormalTok{,}
...
\caption{\label{tab:model1}Table 5. Baseline Model – R² = 0.960}\\
\toprule
...
The exchange rate alone accounts for over 94\% of the variance in the
HPI. A 1\% increase in the USD/TRY rate is associated with approximately
a 1\% increase in the housing price index.
...
\NormalTok{  ) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{kable}\NormalTok{(}\AttributeTok{caption =} \FunctionTok{sprintf}\NormalTok{(}\StringTok{"Table 6. Full Model – R² = \%.3f, Adjusted R² = \%.3f"}\NormalTok{,}
                          \FunctionTok{summary}\NormalTok{(m2)}\SpecialCharTok{$}\NormalTok{r.squared,}
                          \FunctionTok{summary}\NormalTok{(m2)}\SpecialCharTok{$}\NormalTok{adj.r.squared)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable\_styling}\NormalTok{(}\AttributeTok{bootstrap\_options =} \FunctionTok{c}\NormalTok{(}\StringTok{"striped"}\NormalTok{), }\AttributeTok{full\_width =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable}[t]{llrrrll}
\caption{\label{tab:model2}Table 6. Full Model – R² = 0.986, Adjusted R² = 0.986}\\
\toprule
...
Adding sales volume and credit card expenditure raises the adjusted R²
to approximately 0.95. Log(USD/TRY) remains the dominant and most
significant predictor. Log(Sales) enters negatively, confirming that
price increases coincide with falling transaction volumes rather than
rising demand. Log(CC Spend) is positive, reflecting the general
...
No analysis is without constraints, and acknowledging them is an
important part of academic work.

\begin{itemize}
\item
  \textbf{Nominal variables only.} All variables are nominal --- no
  inflation adjustment has been applied. Many series rise together
  simply because of broad inflation, which may inflate the apparent
  strength of correlations. A real House Price Index (deflated by CPI)
  would be a more precise measure of genuine affordability loss.
\item
  \textbf{Correlation is not causation.} The high correlations and
  regression fit confirm strong co-movement, but they do not establish a
  causal direction. Exchange rates and housing prices are likely jointly
  influenced by monetary policy, which we do not model explicitly.
\item
  \textbf{Aggregated credit card data.} The credit card expenditure
  variable aggregates all sectors and acts mainly as a general inflation
  proxy. Sector-specific data (e.g., construction materials) would be
  more theoretically precise.
\item
  \textbf{Static OLS model.} The regression does not account for
  autocorrelation or lagged effects. Time-series models (e.g., VAR or
  Error Correction Models) would better capture the dynamic transmission
  of exchange rate shocks into housing prices over time.
\end{itemize}
