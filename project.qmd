...
We begin by examining the basic descriptive statistics of each key variable to understand their range and scale before looking at relationships between them.
...
eq %>%
  ...
  kable(col.names = c("Expense Category", "Total Spending (Billion TRY)"),
        caption = "Top 8 Credit Card Expenditure Categories – Cumulative Total") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

Retail trade, food, and e-commerce dominate credit card spending by a wide margin. Construction and furniture categories, although smaller in volume, are directly linked to housing demand.
...
```{r}
#| label: trend-hpi
#| fig-cap: "National House Price Index (monthly)"

ggplot(ks_turkey, aes(x = Date, y = Konut_Endeksi)) +
  geom_line(color = "#2c7bb6", linewidth = 0.9) +
  scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  scale_y_continuous(labels = label_comma(accuracy = 1)) +
  labs(title = "National House Price Index Over Time",
       x = NULL, y = "House Price Index") +
  theme_minimal(base_size = 11)
```

The HPI grew modestly through 2019, then accelerated sharply from 2020 onward, with the steepest rise concentrated between 2021 and 2023. This closely mirrors the timing of the most severe TRY depreciation episodes.
...
ggplot(cr_long, aes(x = Date, y = Rate, color = Currency)) +
  ...
  labs(title = "USD/TRY and EUR/TRY Exchange Rates",
       x = NULL, y = "TRY per Foreign Currency", color = "Currency") +
  theme_minimal(base_size = 11)
```

Both exchange rate series follow the same trajectory as the HPI, rising gradually until 2018 and then sharply from 2021. The visual co-movement provides strong initial evidence that currency depreciation is a key driver of housing price inflation.
...
Sales volumes display strong seasonality each year, but the long-run trend has not kept pace with rising prices. After 2022, volumes softened noticeably, suggesting affordability constraints are beginning to suppress demand.
...
All spending categories grew exponentially in nominal terms from 2020 onward. This reflects the broad inflationary environment which also pushes up construction material costs, labour, and land prices — all feeding into higher housing prices.

## Correlation Analysis

To quantify the relationships suggested by the trend charts, we compute correlations between the key variables and examine scatterplots.
...
kable(corr_results, caption = "Pearson Correlations with House Price Index") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

```{r}
#| label: scatter-hpi-usd
#| fig-cap: "House Price Index vs. USD/TRY rate (each point = one month)"

ggplot(combined, aes(x = USD, y = Konut_Endeksi)) +
  geom_point(alpha = 0.5, color = "#2c7bb6") +
  geom_smooth(method = "lm", color = "#d7191c", se = TRUE) +
  scale_x_continuous(labels = label_number(accuracy = 0.1)) +
  scale_y_continuous(labels = label_comma(accuracy = 1)) +
  labs(title = "House Price Index vs. USD/TRY Rate",
       x = "USD/TRY Rate", y = "House Price Index") +
  theme_minimal(base_size = 11)
```

```{r}
#| label: scatter-hpi-sales
#| fig-cap: "House Price Index vs. Monthly Sales (each point = one month)"

ggplot(combined, aes(x = Total_Sales, y = Konut_Endeksi)) +
  ...
  scale_y_continuous(labels = label_comma(accuracy = 1)) +
  labs(title = "House Price Index vs. Monthly Sales Volume",
       x = "Units Sold", y = "House Price Index") +
  theme_minimal(base_size = 11)
```

The HPI correlates extremely strongly with both exchange rates (r \> 0.97) and credit card expenditure, all reflecting the same underlying inflationary driver. The relationship with sales volume is negative at higher price levels — as prices surged after 2021, sales volumes contracted, consistent with buyers being priced out of the market.
...
ks %>%
  ...
  arrange(desc(`Growth (%)`)) %>%
  mutate(across(c(`First HPI`, `Latest HPI`),
                ~ format(.x, big.mark = ","))) %>%
  kable(caption = "HPI Growth by Region (First to Latest Observation)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

İstanbul consistently records the highest absolute index level, while Antalya and coastal regions have also seen strong growth, driven by tourism and migration. Interior Anatolian regions started from a lower base but have experienced comparable percentage increases, confirming the price surge is a nationwide phenomenon.

## Model Fitting

To formally quantify the drivers of housing price growth, we estimate a multiple linear regression model. We apply log-transformations to all variables to handle exponential growth and interpret coefficients as elasticities.
...
as.data.frame(summary(m1)$coefficients) %>%
  ...
  kable(caption = sprintf("Baseline Model – R² = %.3f", summary(m1)$r.squared)) %>%
...
as.data.frame(summary(m2)$coefficients) %>%
  ...
  kable(caption = sprintf("Full Model – R² = %.3f, Adjusted R² = %.3f",
...
```{r}
#| label: fitted-chart
#| fig-cap: "Actual vs. fitted log(HPI) — Full model"
...
The model closely tracks the actual index throughout the sample period, including the inflection point around 2021. Residuals are largest in 2022, which coincides with the most extreme phase of TRY depreciation — possibly reflecting non-linear dynamics or a structural break that a log-linear model cannot fully capture.
